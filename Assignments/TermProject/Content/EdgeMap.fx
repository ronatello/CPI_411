...
float4 pxlSh(VS_OUTPUT In) : COLOR
{
    ...
    /*
    0.0 cause transparency only in drawn lines, why?
    draw normal map edge also so we can compare if line becomes softer due to transparency or intensity
    try rendering in two passes?
    what happens to the depth buffer that opacity only takes effect so close?
    maybe store separately so we can combine the way we want, including the way they suggest
    */
    ...
    */
    ...
    color.a = 1.0f - saturate(pow((1.0f - 0.5f * abs(ztexA - ztexH)), 2.0f) * pow((1.0f - 0.5f * abs(ztexC - ztexF)), 2.0f));
...
float4 zPxlSh(VS_OUTPUT In) : COLOR
{
    ...
    float4 color;
    color.rgb = pow((1.0f - 0.5f * abs(texA - texH)), 2.0f) * pow((1.0f - 0.5f * abs(texC - texF)), 2.0f);
    color.a = 1.0f;
